...
// This function sends messages to the issue_logger on the amqp topic logs/arrivals/issues
function send_feedback(msg){
    var q = 'logs/arrivals/issues';
...
exports.handler = function(context, event) {
    ...
    // write on the issue logger
    send_feedback(currentPlane.model + " " + currentPlane.name + " is " + currentPlane.tag + "! The sensors encountered a " + currentPlane.casualIssue);

    // invoke the webhook
    ifttt.post('plane_malfunction', [currentPlane.model + " - " + currentPlane.name, 
                                      date.getDate() + "/" + (date.getMonth()+1) + "/" + date.getFullYear() + " - " + (date.getHours()+1) + ":" + date.getMinutes(),
                                        currentPlane.casualIssue]);
  };